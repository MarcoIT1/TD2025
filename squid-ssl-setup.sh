...
# Function to install Squid with SSL support
install_squid() {
    echo "Installing Squid proxy server with SSL support..."
    apt install -y squid squid-openssl openssl
    
    # Verify SSL support
    if squid -v | grep -q "openssl"; then
        echo "✅ Squid installed with OpenSSL support"
    else
...
verify_ssl_installation() {
    ...
    # Check if security_file_certgen is running
    if pgrep -f security_file_certgen > /dev/null; then
        echo "✅ SSL certificate generator is running"
    else
        echo "❌ SSL certificate generator is not running"
        exit 1
    fi
    
    # Test HTTP proxy
    echo "Testing HTTP proxy functionality..."
    if timeout 10 curl -x http://localhost:3128 --connect-timeout 10 -s http://httpforever.com/ > /dev/null; then
    ...
    if timeout 10 curl -x localhost:3128 --insecure -s https://httpbin.org/ip > /dev/null; then
        echo "✅ HTTPS interception test successful"
    else
        echo "⚠️ HTTPS interception test failed - check logs"
    fi
...
show_completion() {
    ...
    echo "   • Check SSL processes: ps aux | grep certgen"
    echo ""
